...
io.on("connection", async (socket) => {
    ...
    // cuando una conexion recibe un chat message, msg va a valer al valor que manda el cliente
    socket.on("chat message", async (msg) => {
        ...
        io.emit("chat message", msg, result.insertId.toString(), username); // <- respondemos con el servidor para que emita el mensaje a todos los usuarios
    });
        ...
        try {
            ...
            results.forEach((row) => {
                io.emit(
                    ...
                    row.id.toString(),
                    row.username
                ); // Pasamos los mensajes que ya tenga el usuario
            });
        } catch (e) {
            console.log(e);
            return;
